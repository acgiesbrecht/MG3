...
public class TableControl<R> extends VBox {
    ...
    public TableControl(Class<R> recordClass) {
        ...
        if (Platform.isFxApplicationThread()) {
            ...
            setToolTips();
            /**
             * create custom row factory that can intercept double click on grid
             * row
             */
    ...
    /**
     * Force the table to repaint each row. What this method doing is set the
     * row content to null and then set it back with row's original value. That
     * is the way to trigger
     * {@link Cell#updateItem(java.lang.Object, boolean) updateItem} of every
     * cell in the row.
     */
    public void refresh() {
    ...
    public void save() {
        /**
         * In case there is a cell being edited, call clearSelection() to
         * trigger commitEdit() in the edited cell.
         */
        tblView.getSelectionModel().clearSelection();
    ...
    /**
     * Set a callback to override default error handling. By default, the error
     * is displayed in an error message. The error source might be a failed
     * load, save and delete actions.
     *
     * @param callback
     * @deprecated this function does nothing. Please call
     * {@link #setExceptionHandler(com.panemu.tiwulfx.common.ExceptionHandler) setExceptionHandler}
     * instead
     */
    @Deprecated
    ...
    private void attachWindowVisibilityListener() {
        this.sceneProperty().addListener(new ChangeListener<Scene>() {
            ...
            public void changed(ObservableValue<? extends Scene> ov, Scene t, Scene scene) {
                if (scene != null) {
                    /**
                     * TODO Once this code is executed, the scene listener need
                     * to be removed to avoid potential memory leak. This code
                     * need to be initialized once only.
                     */
                    readColumnPosition();
                    columns.addListener(new ListChangeListener<TableColumn<R, ?>>() {
                        @Override
     ...
     * located in a folder inside user's home directory. Call
     * {@link TiwulFXUtil#setApplicationId(java.lang.String)} to set the folder
     * name. The configurationID must be unique across all TableControl in an
     * application but it is not enforced.
     *
     * @param configurationID must be unique across all TableControls in an
     * application
