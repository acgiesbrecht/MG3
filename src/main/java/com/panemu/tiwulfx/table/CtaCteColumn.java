 ...
 * @author amrullah
 */
public class CtaCteColumn<R, C extends Number> extends BaseColumn<R, C> {
    ...
    public CtaCteColumn(String propertyName, Class<C> clazz) {
        this(propertyName, clazz, 100);
    }

    public CtaCteColumn(String propertyName, Class<C> clazz, double prefWidth) {
        ...
        Callback<TableColumn<R, C>, TableCell<R, C>> cellFactory
            ...
            public TableCell call(TableColumn p) {
                return new CtaCteTableCell<R, C>(CtaCteColumn.this);
            }
        };
    ...
    private StringConverter<C> stringConverter = new StringConverter<C>() {
        ...
        public String toString(C value) {
            return value == null ? getNullLabel() : FormatCtaCte.format(value.intValue());
        }
        ...
        public C fromString(String stringValue) {
            ...
            return searchInputControl.castToExpectedType(stringValue.replaceAll("-", "").replaceAll("/", ""));
        }
    };
     ...
     * character. To change application wide value, see
    ...
    /**
     * Check if this NumberField allow negative value. Default value is
     * {@link TiwulFXUtil#DEFAULT_NEGATIVE_ALLOWED}
     *
